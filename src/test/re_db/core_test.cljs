...
(do
  ...
  (deftest basic
    (let [conn (d/create-conn {:dog {:db/index true}})
      ...
      (is (= 1 (count (read/entities conn [[:occupation "teacher"]])))
          "Query on non-indexed attr")

      (d/transact! conn [{:db/id "fred" :occupation "teacher"}])

      (is (= 2 (count (read/entities conn [[:occupation "teacher"]])))
      ...
      (is (empty? (read/entity-ids conn [[:dog "herman"]]))
  ...
  (deftest refs
    (let [db (doto (d/create-conn {:owner {:db/valueType :db.type/ref}})
      ...
      (is (= {:db/id "fred"
              :name "Fred"
              :_owner #{"ball"}} (read/touch db (read/get db "fred")))
